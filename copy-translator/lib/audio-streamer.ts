...
export class AudioStreamer {
  ...
  // Destination for capturing stream (to send to remote peer)
  ...
  constructor(public context: AudioContext) {
    ...
    // Create a destination node to capture the audio as a MediaStream
    this.streamDestination = this.context.createMediaStreamDestination();
    
    // Connect gain to the stream destination (always active)
    this.gainNode.connect(this.streamDestination);
    
    // Default: DO NOT connect to local speakers (context.destination) 
    // to prevent the local user from hearing their own translation loop.
    // Use setLocalPlayback(true) to enable monitoring.
    
    this.addPCM16 = this.addPCM16.bind(this);
  }
  
  /**
   * Toggle local playback (monitoring). 
   * If true, connects to speakers. If false, disconnects from speakers.
   */
  setLocalPlayback(enabled: boolean) {
    if (enabled) {
      // Connect to local speakers
      try { this.gainNode.connect(this.context.destination); } catch(e) {}
    } else {
      // Disconnect from local speakers (but keep streamDestination)
      try { this.gainNode.disconnect(this.context.destination); } catch(e) {}
    }
  }
  
  /**
   * Returns the MediaStream of the translated audio.
   * capable of being used as a source for WebRTC or Recorder.
   */
